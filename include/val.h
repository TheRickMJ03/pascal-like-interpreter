...
public:
    ...
    // numeric overloaded add this to op                COMPLETED
    Value operator+(const Value& op) const;
    
    // numeric overloaded subtract op from this             COMPLETED
    Value operator-(const Value& op) const;                 
    
    // numeric overloaded multiply this by op                   COMPLETED
    Value operator*(const Value& op) const;
    
    // numeric overloaded divide this by oper               COMPLETED          
    Value operator/(const Value& op) const;
    
    //string concatenation of this with op                      COMPLETED
    Value Catenate(const Value & op) const;
    
    //compute the value of this raised to the exponent op       COMPLETED
    Value Power(const Value & op) const;                    
    
    //overloaded equality operator of this with op              COMPLETED
    Value operator==(const Value& op) const;
	//overloaded greater than operator of this with op             COMPLETED
	Value operator>(const Value& op) const;
	//overloaded less than operator of this with op             COMPLETED
	Value operator<(const Value& op) const;
